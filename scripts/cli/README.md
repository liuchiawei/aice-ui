...
本目錄為 Aice UI 的 CLI 實作區，提供 `npx aice-ui add <component>`、`npx aice-ui list` 等指令，讓使用者從 [Aice UI Registry](https://aice-ui.vercel.app) 安裝元件到自己的專案。
...
| **`package.json`** | 定義 `bin["aice-ui"] = "dist/index.js"`、`scripts["build:cli"]`、`files` 含 `dist`，發佈時僅會帶上 CLI 產物。 |
...
```bash
# 安裝單一元件
npx aice-ui add copy-button

# 安裝多個元件
npx aice-ui add copy-button theme-switch glass-surface
...
發佈後使用者即可直接使用 `npx aice-ui add <name>`，無需安裝到全域。
