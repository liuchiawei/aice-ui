...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  const activityTargetRef = useRef<number>(0);
  ...
  return (
    ...
    >
      ...
      {isLoading && (
        <div
          className="absolute inset-0 flex items-center justify-center bg-muted/80 rounded-[inherit]"
          aria-hidden
        >
          <div className="size-8 border-2 border-muted-foreground/30 border-t-muted-foreground rounded-full animate-spin" />
        </div>
      )}
    </div>
  );
