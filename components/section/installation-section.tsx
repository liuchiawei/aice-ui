...
import { getInstallCommands, getShadcnCliCommands } from "@/lib/installation";
...
export function InstallationSection({ slug }: InstallationSectionProps) {
  ...
  const shadcnCommands = getShadcnCliCommands(slug);
  ...
  return (
    <section className="flex flex-col gap-3 border-t border-border pt-6">
      ...
      <div className="flex flex-col gap-2">
        <p className="text-sm font-medium">{t("install.shadcnLabel")}</p>
        <Tabs defaultValue="pnpm" className="w-full">
          <TabsList>
            {PACKAGE_MANAGERS.map((pm) => (
              <TabsTrigger key={pm} value={pm}>
